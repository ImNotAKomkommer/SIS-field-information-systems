...
def create_user(full_name, email, password, english_level, learning_goal):
    ...
    try:
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """, (full_name, email, hash_password(password), english_level, learning_goal,
...
class App(tk.Tk):
    ...
    def card(self, parent, padx=22, pady=14):
        frame = tk.Frame(parent, bg=CARD, highlightbackground=BORDER, highlightthickness=1)
        frame.pack(fill="both", expand=False, padx=padx, pady=pady)
        return frame
    ...
    def show_login(self):
        ...
        box = tk.Frame(outer, bg=CARD, padx=34, pady=28, highlightbackground=BORDER, highlightthickness=1)
        box.pack()
    ...
    def show_register(self):
        ...
        box = tk.Frame(outer, bg=CARD, padx=34, pady=26, highlightbackground=BORDER, highlightthickness=1)
        box.pack(pady=35)
    ...
    def sidebar(self, parent):
        side = tk.Frame(parent, bg="#0b1220", width=230)
        ...
        for text, cmd in buttons:
            DarkButton(side, text, cmd, bg="#172033").pack(fill="x", padx=16, pady=5)
    ...
    def page_base(self, title, subtitle=""):
        ...
        tk.Label(main, text=title, font=FONT_TITLE, fg=TEXT, bg=BG).pack(anchor="w", padx=28, pady=(24, 4))
        if subtitle:
            tk.Label(main, text=subtitle, font=FONT_BODY, fg=MUTED, bg=BG).pack(anchor="w", padx=28, pady=(0, 10))

        return main

    def show_dashboard(self):
        main = self.page_base("Dashboard", "Project prototype with authentication, profile, tutor booking, lecture, quiz and AI assistant.")
        ...
        cards = [
            ...
            ("Lecture access", "Unlocked" if lecture_unlocked else "Locked until booking", GREEN if lecture_unlocked else RED),
        ...
        for i, (name, value, color) in enumerate(cards):
            c = tk.Frame(grid, bg=CARD, padx=18, pady=16, highlightbackground=BORDER, highlightthickness=1)
            c.grid(row=0, column=i, sticky="nsew", padx=6)
            tk.Label(c, text=name, font=FONT_BODY, fg=MUTED, bg=CARD).pack(anchor="w")
            tk.Label(c, text=value, font=("Segoe UI", 16, "bold"), fg=color, bg=CARD).pack(anchor="w", pady=(8, 0))
            grid.columnconfigure(i, weight=1)

        info = self.card(main)
        self.label(info, "How the app works", FONT_H2).pack(anchor="w", padx=18, pady=(16, 8))
        text = (
            "1. Create an account and log in.\n"
            "2. Open Tutor Search and book a lesson with the English tutor.\n"
            "3. After booking, the English lecture becomes available in your profile.\n"
            "4. Read the lecture and mark it as completed.\n"
            "5. After completing the lecture, the quiz becomes available.\n"
            "6. Use the AI assistant to ask questions about grammar, tutor booking or learning plans."
        )
        self.label(info, text, FONT_BODY, wraplength=760).pack(anchor="w", padx=18, pady=(0, 18))

        bookings = get_bookings(self.current_user_id)
        if bookings:
            bcard = self.card(main)
            self.label(bcard, "Latest booking", FONT_H2).pack(anchor="w", padx=18, pady=(16, 8))
    ...
    def show_profile(self):
        ...
        c = self.card(main)
        self.label(c, "User Information", FONT_H2).pack(anchor="w", padx=18, pady=(16, 8))
        info = (
            f"Full name: {user[1]}\n"
            f"Email: {user[2]}\n"
        ...
        self.label(c, info, FONT_BODY, wraplength=800).pack(anchor="w", padx=18, pady=(0, 16))

        p = self.card(main)
        self.label(p, "Learning Progress", FONT_H2).pack(anchor="w", padx=18, pady=(16, 8))
        status = (
        ...
        self.label(p, status, FONT_BODY).pack(anchor="w", padx=18, pady=(0, 16))

        if lecture_unlocked:
            DarkButton(p, "Open lecture", self.show_lecture).pack(anchor="w", padx=18, pady=(0, 18))
        else:
            self.label(p, "Book a lesson with the tutor to unlock the lecture.", FONT_BODY, fg=YELLOW).pack(anchor="w", padx=18, pady=(0, 18))
        ...
        return [
            {
                ...
                "bio": (
                    ...
                    "My lessons are focused on simple explanations, real examples, speaking practice, "
                    "grammar exercises and personal feedback. I mainly work with beginner and intermediate students."
                )
            },
            {
                "id": 2,
            ...
            },
            {
                ...
                "bio": (
                    ...
                    "He is currently busy with existing students and is not available for new lesson requests."
                )
            }
        ]
    ...
    def show_tutor(self):
        ...
        search_card = tk.Frame(main, bg=CARD, padx=18, pady=16, highlightbackground=BORDER, highlightthickness=1)
        search_card.pack(fill="x", padx=22, pady=14)
        ...
        search_entry.pack(fill="x", pady=(6, 10), ipady=7)

        button_row = tk.Frame(search_card, bg=CARD)
        button_row.pack(fill="x")

        # Scrollable tutor list
        list_outer = tk.Frame(main, bg=BG)
        list_outer.pack(fill="both", expand=True, padx=22, pady=(0, 14))
        ...
        list_area = tk.Frame(canvas, bg=BG)
        ...
        canvas_window = canvas.create_window((0, 0), window=list_area, anchor="nw")
        canvas.configure(yscrollcommand=scrollbar.set)

        def resize_list_area(event):
            canvas.itemconfig(canvas_window, width=event.width)

        canvas.bind("<Configure>", resize_list_area)
        ...
        def render_list():
            for widget in list_area.winfo_children():
                widget.destroy()
            ...
            if not tutors:
                empty = tk.Frame(list_area, bg=CARD, padx=18, pady=18, highlightbackground=BORDER, highlightthickness=1)
                empty.pack(fill="x", pady=(0, 10))
                self.label(empty, "No tutors found.", FONT_H2).pack(anchor="w")
                return

            for tutor in tutors:
                card = tk.Frame(list_area, bg=CARD, padx=16, pady=14, highlightbackground=BORDER, highlightthickness=1, cursor="hand2")
                card.pack(fill="x", pady=(0, 12))

                left = tk.Frame(card, bg=CARD, cursor="hand2")
                left.pack(side="left", fill="both", expand=True)

                top_row = tk.Frame(left, bg=CARD, cursor="hand2")
                top_row.pack(fill="x")

                photo = tk.Canvas(top_row, width=100, height=100, bg=CARD_2, highlightthickness=1, highlightbackground=BORDER, cursor="hand2")
                photo.pack(side="left", padx=(0, 16), anchor="n")
                photo.create_oval(28, 14, 72, 58, fill="#64748b", outline="")
                photo.create_rectangle(20, 58, 80, 90, fill="#475569", outline="")
                photo.create_text(50, 86, text="Tutor", fill=TEXT, font=("Segoe UI", 8, "bold"))

                info = tk.Frame(top_row, bg=CARD, cursor="hand2")
                info.pack(side="left", fill="both", expand=True)

                status_color = GREEN if tutor["status"] == "Available" else RED

                tk.Label(info, text=tutor["name"], bg=CARD, fg=TEXT, font=("Segoe UI", 15, "bold"), cursor="hand2").pack(anchor="w")
                tk.Label(info, text=tutor["subject"], bg=CARD, fg=ACCENT, font=FONT_BODY, cursor="hand2").pack(anchor="w", pady=(2, 4))
                tk.Label(
                    info,
                ...
                # Use a separate full-width label below top row so the bio has enough width.
                bio_label = tk.Label(
                    left,
                    text=tutor["bio"],
                    bg=CARD,
                ...
                bio_label.pack(fill="x", pady=(10, 0))

                btn = DarkButton(card, "Open profile", lambda t=tutor: open_selected_tutor(t), bg="#334155")
                btn.pack(side="right", padx=(14, 0), anchor="n")

                # Make the whole card clickable
                clickable_widgets = [card, left, top_row, photo, info, bio_label]
    ...
    def show_tutor_profile(self, tutor):
        ...
        # Scrollable profile page to prevent text from being cut on small screens
        outer_canvas = tk.Canvas(main, bg=BG, highlightthickness=0)
        outer_scrollbar = tk.Scrollbar(main, orient="vertical", command=outer_canvas.yview)
        content_frame = tk.Frame(outer_canvas, bg=BG)

        content_frame.bind(
            "<Configure>",
            lambda event: outer_canvas.configure(scrollregion=outer_canvas.bbox("all"))
        )

        canvas_window = outer_canvas.create_window((0, 0), window=content_frame, anchor="nw")
        outer_canvas.configure(yscrollcommand=outer_scrollbar.set)

        def resize_content(event):
            outer_canvas.itemconfig(canvas_window, width=event.width)

        outer_canvas.bind("<Configure>", resize_content)

        outer_canvas.pack(side="left", fill="both", expand=True, padx=22, pady=14)
        outer_scrollbar.pack(side="right", fill="y", pady=14)

        def on_profile_mousewheel(event):
            outer_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

        outer_canvas.bind_all("<MouseWheel>", on_profile_mousewheel)

        c = tk.Frame(content_frame, bg=CARD, padx=20, pady=20, highlightbackground=BORDER, highlightthickness=1)
        c.pack(fill="both", expand=True)

        top = tk.Frame(c, bg=CARD)
        top.pack(fill="x")

        photo = tk.Canvas(top, width=140, height=140, bg=CARD_2, highlightthickness=1, highlightbackground=BORDER)
        photo.pack(side="left", padx=(0, 22), anchor="n")
        photo.create_oval(38, 18, 102, 82, fill="#64748b", outline="")
        photo.create_rectangle(30, 82, 110, 128, fill="#475569", outline="")
        photo.create_text(70, 124, text="Tutor", fill=TEXT, font=("Segoe UI", 10, "bold"))
        ...
        tk.Label(details, text=tutor["name"], bg=CARD, fg=TEXT, font=("Segoe UI", 20, "bold")).pack(anchor="w")
        tk.Label(details, text=tutor["subject"], bg=CARD, fg=ACCENT, font=FONT_BODY).pack(anchor="w", pady=(4, 8))
        tk.Label(details, text=f"Status: {tutor['status']}", bg=CARD, fg=status_color, font=("Segoe UI", 11, "bold")).pack(anchor="w", pady=(0, 6))
        ...
        bio_box = tk.Frame(c, bg=CARD_2, padx=14, pady=12, highlightbackground=BORDER, highlightthickness=1)
        bio_box.pack(fill="x", pady=(18, 14))
        ...
        if tutor["status"] != "Available":
            locked = tk.Frame(c, bg=CARD_2, padx=14, pady=12, highlightbackground=BORDER, highlightthickness=1)
            locked.pack(fill="x", pady=(0, 14))
            tk.Label(
                locked,
        ...
        if self.user_has_booking_with_tutor(tutor["name"]):
            already = tk.Frame(c, bg=CARD_2, padx=14, pady=12, highlightbackground=BORDER, highlightthickness=1)
            already.pack(fill="x", pady=(0, 14))
            tk.Label(
                already,
            ...
            DarkButton(c, "Open lecture", lambda: (outer_canvas.unbind_all("<MouseWheel>"), self.show_lecture())).pack(anchor="w")
            return

        form = tk.Frame(c, bg=CARD_2, padx=14, pady=12, highlightbackground=BORDER, highlightthickness=1)
        ...
        preferred.pack(fill="x", pady=(4, 10), ipady=7)
        ...
        message_box.pack(fill="x", pady=(4, 12))
    ...
    def show_lecture(self):
        ...
        main = self.page_base("English Lecture", "Lecture becomes available after booking a lesson with the tutor.")

        if not lecture_unlocked:
            c = self.card(main)
            self.label(c, "Lecture is locked", FONT_H2, fg=RED).pack(anchor="w", padx=18, pady=(16, 8))
            self.label(c, "Please book a lesson with the tutor first. After booking, the lecture will appear in your profile.", FONT_BODY, wraplength=780).pack(anchor="w", padx=18, pady=(0, 18))
            DarkButton(c, "Go to Tutor Search", self.show_tutor).pack(anchor="w", padx=18, pady=(0, 18))
            return

        c = self.card(main)
        self.label(c, "Test Lecture: Present Simple in English", FONT_H2).pack(anchor="w", padx=18, pady=(16, 8))
        ...
        lecture = (
            ...
            "In this lesson, the main rule is simple: use the base verb for I, you, we and they, but add -s or -es for he, she and it in positive sentences."
        )

        txt = tk.Text(c, bg="#0b1220", fg=TEXT, insertbackground=TEXT, relief="flat", height=20, wrap="word", font=FONT_BODY, highlightbackground=BORDER, highlightthickness=1)
        txt.insert("1.0", lecture)
    ...
    def show_quiz(self):
        ...
        if not lecture_unlocked:
            c = self.card(main)
            self.label(c, "Quiz is locked", FONT_H2, fg=RED).pack(anchor="w", padx=18, pady=(16, 8))
            self.label(c, "Book a lesson first, then read the lecture.", FONT_BODY).pack(anchor="w", padx=18, pady=(0, 18))
            return

        if not lecture_completed:
            c = self.card(main)
            self.label(c, "Quiz is locked until the lecture is completed", FONT_H2, fg=YELLOW).pack(anchor="w", padx=18, pady=(16, 8))
            self.label(c, "Please read the lecture and press the completion button before starting the quiz.", FONT_BODY, wraplength=780).pack(anchor="w", padx=18, pady=(0, 18))
            DarkButton(c, "Open lecture", self.show_lecture).pack(anchor="w", padx=18, pady=(0, 18))
            return
        ...
        outer = tk.Frame(main, bg=CARD, highlightbackground=BORDER, highlightthickness=1)
        outer.pack(fill="both", expand=True, padx=22, pady=14)

        self.label(outer, "Present Simple Quiz", FONT_H2).pack(anchor="w", padx=18, pady=(16, 8))

        # Scrollable area for quiz questions
        ...
        for q, options, answer in questions:
            block = tk.Frame(scroll_frame, bg=CARD_2, padx=12, pady=8, highlightbackground=BORDER, highlightthickness=1)
        ...
        button_area = tk.Frame(scroll_frame, bg=CARD)
        button_area.pack(fill="x", pady=(12, 18))

        def submit():
            unanswered = [1 for var, ans in vars_ if not var.get()]
            if unanswered:
        ...
        DarkButton(button_area, "Submit quiz", submit).pack(anchor="w")


    def show_ai(self):
        main = self.page_base("AI Assistant", "Prototype assistant for English learning and app navigation.")

        c = self.card(main)
        self.label(c, "Ask the AI Assistant", FONT_H2).pack(anchor="w", padx=18, pady=(16, 8))

        chat = tk.Text(c, bg="#0b1220", fg=TEXT, insertbackground=TEXT, relief="flat", height=18, wrap="word", font=FONT_BODY, highlightbackground=BORDER, highlightthickness=1)
        chat.pack(fill="both", expand=True, padx=18, pady=(0, 12))
        ...
        DarkButton(btn_row, "Clear", lambda: entry.delete(0, "end"), bg="#334155").pack(side="left", padx=8)
